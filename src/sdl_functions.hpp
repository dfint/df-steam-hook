...
typedef SDL_Surface* (*SDL_CreateRGBSurface_)(Uint32 flags, int width,
                                              int height, int depth,
                                              Uint32 Rmask, Uint32 Gmask,
                                              Uint32 Bmask, Uint32 Amask);
typedef Uint32 (*SDL_MapRGB_)(const SDL_PixelFormat* const format,
...
typedef int (*SDL_UpperBlit_)(SDL_Surface* src, SDL_Rect* srcrect,
...
void LoadSDLAdresses()
{
  ...
  if (!hModule) {
    spdlog::error("Unale to laod SDL.dll");
    exit(2);
  }

  SDL_DisplayFormat_ptr =
    (SDL_DisplayFormat_)GetProcAddress(hModule, "SDL_DisplayFormat");
  SDL_CreateRGBSurface_ptr =
    (SDL_CreateRGBSurface_)GetProcAddress(hModule, "SDL_CreateRGBSurface");

  SDL_MapRGB_ptr = (SDL_MapRGB_)GetProcAddress(hModule, "SDL_MapRGB");
  SDL_FreeSurface_ptr =
    (SDL_FreeSurface_)GetProcAddress(hModule, "SDL_FreeSurface");
  ...
  SDL_GetVideoSurface_ptr =
    (SDL_GetVideoSurface_)GetProcAddress(hModule, "SDL_GetVideoSurface");
}